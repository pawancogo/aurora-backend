...
SPRINTS = [
  ...
  { number: 13, title: "Customer Profile & Address Book", status: "completed",
    goal: "Profile editing and a real, reusable address book (add/edit/delete, default, Home/Office/Other) — separate from the one-off address a shopper types at checkout. Corresponds to canonical ROADMAP.md Sprint 8 — the most-flagged gap in the project's own docs (ROADMAP.md marked it pending by name, the storefront's /account page literally said \"arrives in Sprint 7,\" and the account menu's Addresses link was disabled). OrderAddress was deliberately designed as a checkout-time snapshot precisely so it never had to depend on this book existing, which is why checkout shipped years ahead of it.",
    dependencies: "Sprint 2", estimate: "1 week",
    features: [
      { area: :backend, title: "Address book model + one default per customer",
        description: "<p>A shopper can save multiple addresses — Home, Office, or Other — and exactly one of them is always \"the default\" at any given time. Marking a different one default automatically un-defaults whichever one held that spot before; deleting the current default automatically promotes another one, so a shopper is never left with zero defaults just because they cleaned up their list.</p><p><strong>Where to see it:</strong> storefront → Account menu → Addresses.</p>",
        technical_description: "<p>New <code>Address</code> model (<code>customer_id</code>, <code>address_type</code> enum home/office/other, <code>is_default</code>, plus the same field shape as <code>OrderAddress</code>: full_name/phone/line1/line2/city/state/postal_code/country). <code>before_save :demote_other_defaults</code> un-defaults every other address for the same customer whenever one is saved as default; <code>after_destroy :promote_next_default</code> promotes the most recently updated remaining address if the deleted one was the default. No RBAC permission keys on the customer-facing side — a customer is only ever authorized over their own <code>current_customer.addresses</code> scope, so touching another customer's address 404s exactly like it not existing.</p>" },
      { area: :backend, title: "Profile + address book API",
        description: "<p>The endpoints a shopper's own account screens call: update your name/phone, and list/add/edit/delete/default your saved addresses.</p><p><strong>Where to see it:</strong> not a UI itself — powers the Account and Addresses pages below.</p>",
        technical_description: "<p><code>PATCH /api/v1/customer/auth/me</code> added alongside the existing <code>GET</code> (<code>Api::V1::Customer::MeController#update</code>). New <code>Api::V1::Customer::AddressesController</code> (index/create/update/destroy + a <code>default</code> member action) under <code>/api/v1/customer/addresses</code>, following the same <code>CustomerAuthentication</code> + <code>authenticate_customer!</code> pattern as every other customer endpoint. A new address is auto-defaulted only when it's the customer's first (checked via a plain <code>.none?</code> query taken <em>before</em> building the new record — building via the association loads its target and would otherwise make that check see the just-built unsaved record and never treat it as \"first\").</p>" },
      { area: :frontend, title: "Real profile + address book pages",
        description: "<p>The storefront's Account page went from a static \"arrives later\" placeholder to a real profile-edit form (name/phone), and a brand new Addresses page lets a shopper add, edit, delete, and set a default among their saved addresses — the same Home/Office/Other typing as the backend, with an inline form for both adding and editing so there's no separate page to navigate to for a form this small.</p><p><strong>Where to see it:</strong> storefront → Account menu → My account (profile form) and → Addresses (the book itself, now enabled instead of showing its old \"soon\" badge).</p>",
        technical_description: "<p><code>src/lib/api/addresses.ts</code> (zod schema + CRUD calls, mirrors <code>orders.ts</code>'s shape) and <code>updateProfile()</code> added to <code>src/lib/auth/api.ts</code>. <code>AuthProvider</code> gained <code>updateCustomer()</code> to refresh the in-memory customer after a profile edit, since that response carries an updated customer but no new tokens (so <code>setSession</code> doesn't fit). <code>src/components/account/address-form.tsx</code> is the shared RHF+Zod form for both add and edit, reusing the same field shape as checkout's own address form. <code>UserMenu</code>'s Addresses link had its <code>soon: \"S7\"</code> badge removed — the same de-flagging its Orders link went through when Sprint 9 shipped.</p>" },
      { area: :admin, title: "Read-only address book on the customer page",
        description: "<p>Staff can see a customer's saved addresses — which one's the default, and the full address for each — right on that customer's own admin page, without being able to edit them (address book management stays customer-only, matching the original \"view read-only\" scope).</p><p><strong>Where to see it:</strong> Admin → Customers → open any customer with saved addresses.</p>",
        technical_description: "<p><code>Admin::CustomersController#show</code> loads <code>@addresses = @customer.addresses.order(is_default: :desc, id: :asc)</code>; <code>show.html.erb</code> renders them via the existing <code>admin/ui/table</code> partial. Gated by the existing <code>customers.read</code> permission — no new RBAC needed.</p>" }
    ] },
