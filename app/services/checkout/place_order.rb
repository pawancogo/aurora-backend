...
module Checkout
  ...
  # decrements on-hand inventory, and clears the cart. No payment step exists
  # yet (that's a later sprint) — placing an order confirms it outright.
  class PlaceOrder
    ...
    def call
      ...
      Order.transaction do
        ...
        items.each { |item| decrement_inventory!(item) }
    ...
    # Decrements on-hand for whatever's truly on-hand; never pushes it
    # negative (AdjustStock's own invariant) — a backorderable item sold
    # beyond on-hand simply isn't ledgered past what physically existed.
    def decrement_inventory!(item)
      ...
      return unless decrement.positive?

      Inventory::AdjustStock.new(
        inventory_item: inventory_item, quantity: -decrement, reason: "sale"
      ).call
    end
