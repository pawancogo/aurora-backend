...
- **Active sprint:** Sprint 10 — Payments (Razorpay) 🚧 in progress (see the admin Roadmap tracker, `/admin/sprints/10`, for the detailed writeup)
- **Status:** Sprints 1–9 done (see the numbering note below — canonical Sprint 8, Customer Profile & Address Book, is the one genuinely still pending). Sprint 10's backend + frontend integration is built and verified live against a real Razorpay test account (order creation, signature verification, reservation/fulfillment, webhook, retry — all confirmed); the one remaining step is clicking through an actual test payment inside Razorpay's hosted widget in a real browser.
...
- **Sprint tracking has moved off this file.** The backend now has a DB-backed admin **Roadmap** (`/admin/sprints`, `Sprint`/`SprintFeature` models) — viewable and editable by every admin role, with a plain-language description and a separate technical description per feature. It's seeded with the Sprint 1–9 history below so it doesn't start empty. This markdown log is kept for historical narrative detail (screenshots, gotchas, exact verification runs) that doesn't belong in the tracker; day-to-day "what shipped in sprint N" now lives in the admin UI, not here.
...
### Post-Sprint-9: Admin Roadmap tracker (2026-08-11)

Sprint tracking moved off static markdown into the admin portal, so any admin can see (and edit) what shipped without a repo checkout:

- **DB (2 tables):** `sprints` (number/title/goal/status enum/dependencies/estimate/dates) + `sprint_features` (area enum, title, and **two** separate HTML bodies — `description` for plain-language "what it does," `technical_description` for implementation detail — since PMs/stakeholders and engineers read completely differently). Both sanitized on save (`sanitize` allowlist) and again at render time.
- **Permissions:** new `roadmap.read`/`roadmap.manage` — deliberately granted to **every** seeded role (not just admin/super_admin), matching the requirement that any admin can view and edit the tracker.
- **Admin UI** (`/admin/sprints`): each sprint is a native `<details>` accordion card — collapsed by default, expands to show its goal/dependencies/estimate and every feature's two description sections, addressing the earlier gap where a sprint number/name alone didn't convey what was actually inside. Sprint-level Edit/Delete sit in an overlay *outside* the `<details>` element (a `<details>` hides everything but its `<summary>` while collapsed — action buttons placed inside would vanish when collapsed). Feature CRUD reuses the same full-page form convention as the rest of the admin (not modals, so validation-error re-rendering stays simple); a reusable rich-text editor (`admin/ui/_richtext` — contenteditable + toolbar, no external JS dependency, plain-text paste only) backs both description fields. Create/update/delete redirects carry a `#sprint-<id>` anchor + a small JS snippet re-opens that sprint's accordion, so editing a feature doesn't dump the admin back at a fully collapsed list.
- **Seeded** with the Sprint 1–9 history (39 features) so the tracker launches with real content instead of empty; the seed loop always syncs attributes (not just on first create) so edits to the seed data show up on re-seed.
- Backend port default changed to **3001** (`config/puma.rb`) so `bin/rails server`/`bundle exec rails s` need no `-p` flag; frontend already defaulted to 3000 via Next.js.

Verification: backend **287 RSpec** (15 new: Sprint/SprintFeature models + admin sprints/sprint_features request specs incl. permission gating) / RuboCop clean (303 files) / Brakeman 0; live real-click — expand/collapse persists action buttons, added/edited/deleted a feature with both rich-text fields, search + status filter narrow the list, hash-anchor reopens the right sprint after a redirect.
